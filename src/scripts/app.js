...
window._InfoDisplayModule = {
    ...
    querySingleData:function(requestParam){
        ...
        $.ajax({
            ...
            success: function(jsonResponse){
                var jsonStr = JsonUti.convertToString(jsonResponse);//JSON.stringify(jsonResponse);
                var prettyStr = prettyPrintOne(jsonStr);

                var $codeViewContainer = $(".result-show-block > pre");
                $codeViewContainer.html(prettyStr).attr("class","prettyprit");
